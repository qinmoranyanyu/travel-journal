...
<p align="center"><strong>把散落在相册里的旅行照片，编成一册有时间、地点和诗意的视觉纪行。</strong></p>

旅迹编年不是给照片套一层滤镜。它会理解一整段旅程：按拍摄时间整理照片，过滤近似画面，从中挑出更有叙事价值的瞬间，结合 EXIF 地点组织章节与路线，再按用户选择的视觉风格重新创作每张入选照片，最后交付可离线浏览的 Web 相册、朋友圈章节长图和分享 ZIP。
...
- **一张照片，两层文案**：旅行手绘使用 4–30 字中文图内旁白并保留日期；拾景纸刊使用主词、观察句、双语回声与事实小字组成编辑式微文案，极简纸刊让核心词、碎片句与档案微字共同参与构图，并逐页轮换字体材料和排版。Web 相册和分享长图仍使用独立中文诗行，所有诗行顺序相连，在语意自然的前提下按小节押宽韵。
- **三种固定版本的视觉风格**：旅行手绘使用白纸、水彩与手写笔记；拾景纸刊结合真实照片、撕纸和抽象插画；极简纸刊使用旧纸留白、微型主体与实验排版。风格在任务创建后锁定，暂停、恢复和重试均保持一致。
...
| `IMAGE_GENERATION_TIMEOUT_SECONDS` | 单张图片流等待完成事件的最长秒数 | `360` |
...
项目固定使用以下 MIT License 上游版本，运行时不会自动拉取更新：

- `third_party/photo-revival`：`ca4c3c6c0f812355bd6d815d8a78652db801b7f1`
- `third_party/scenes-gathered-zine-v1-3`：`e764b7fd243d7cc501723b9d325279bf6dd852c2`
- `third_party/gc-minimal-zine-poster-v0-1`：`4cb0396ad4e834019f753b37e1c4f415f5e02026`
